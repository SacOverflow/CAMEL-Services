...
export default function LanguageOption({ lang }: { lang: string }) {
	...
	return (
		<>
			<div
				onClick={() => {
					setLanguage(languages[lang]['value']).then(res => {
						if (res) {
							console.info(
								'updated user lang pref to ',
								languages[lang]['value'],
							);
						} else {
							console.warn('Failed to create lanuage pref');
						}
					});
				}}
				className={lang + ' language'}
			>
				{languages[lang]['flagSvg']}
				<h2 className="ml-14">{languages[lang]['name']}</h2>
			</div>
		</>
	);
}
